...
def graficas():
    ...
    df = datos[["RUT", "NOMBRE", "NP"]]

    df = datos.rename(columns={"RUT": "RUT", "NOMBRE": "NOMBRE", "NP": "NP"})

    print("51. Impresión normal")

    print(df.sample(contar))

    print("52. Impresion Histograma")

    df.NP.plot.hist()

    matplotlib.pyplot.show()

    print("53. Impresión histograma frecuencia")

    df.NP.plot.hist(bins=10, xlim=(0, 7))

    matplotlib.pyplot.show()

    print("54. Impresión histograma dispersion")

    df.plot.scatter(x="RUT", y="NP", alpha=0.5)

    matplotlib.pyplot.show()

    print("55. Impresión histograma X e Y")

    NOMBRE = df.groupby("NOMBRE")["NP"].mean()

    NOMBRE.head(10).plot.barh()

    matplotlib.pyplot.show()

    print("56. Impresión dispersion impacto")

    df["N"] = pandas.qcut(df.NP, 5)

    df.boxplot(column="NP", by="N", figsize=(8, 6))

    matplotlib.pyplot.show()

    print("57. Impresión gráfico de torta")

    df.NP.value_counts().plot.pie()
