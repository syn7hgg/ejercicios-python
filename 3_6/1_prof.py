...
def separar_mes(nombre, mes):
    archivo = open(nombre, "r")

    datos = archivo.readlines()

    archivo_mes = open(mes + "-solo.txt", "w")

    archivo_mes_otro = open("meses_resto.txt", "w")
    ...
    for reg in datos:
        ...
        if j[3:9] == mes:
            ...
            archivo_mes.write(linea)

        else:

            linea = j + "#" + num

            archivo_mes_otro.write(linea)

    archivo_mes.close()

    archivo_mes_otro.close()
